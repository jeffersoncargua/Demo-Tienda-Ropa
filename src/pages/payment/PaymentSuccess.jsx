...
export const PaymentSuccess = () => {
	...
	const GetUser = useCallback(async (identificacion) => {
		const response = await BuscarUsuario(identificacion);
		if (response.isSuccess) {
			setCostumer(response.result);
			setMessage(
				"Su compra se ha realizado con éxito. Por favor, revise su correo para descargar su factura. !Gracias por su preferencia!.",
			);
		} else {
			setCostumer({});
			setMessage("");
	...
	return (
		<div
			className={` ${costumer.identificacion ? "flex" : "hidden"} w-[90%] md:w-[80%]  flex-col mx-auto min-h-full space-y-5 my-20`}
		>
			{loading ? (
				<Loading />
			) : (
				<>
					<div className="flex items-center justify-center w-36 h-36 sm:w-48 sm:h-48 md:w-56 md:h-56 rounded-full bg-green-500/50 mx-auto p-5">
						<svg
							...
							fill="currentColor"
							className="bi bi-cart-check-fill mx-auto my-auto w-20 h-20 sm:w-28 sm:h-28 md:w-36 md:h-36 text-green-700"
							viewBox="0 0 16 16"
						>
