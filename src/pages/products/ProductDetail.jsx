...
export const ProductDetail = () => {
	...
	const handleDecrement = () => {
		if (cantidad === 0) {
			toast.info("La cantidad del producto es cero. No puede llevar cero productos");
		} else {
			setCantidad((cantidad) => cantidad - 1);
		}
	};
	...
	const AddToCart = () => {
		if (cantidad > 0) {
			const item = {
				...
				descripcion:
					`${producto.descripcion} | Talla: ${tallaRef.current.value}`,
			...
			{loading ? (
				...
				:(
					<div className="w-full flex flex-col my-10 space-y-10 text-xs md:text-sm">
						<span>
							<svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="bi bi-x-octagon-fill mx-auto w-22 h-22" viewBox="0 0 16 16">
								<path d="M11.46.146A.5.5 0 0 0 11.107 0H4.893a.5.5 0 0 0-.353.146L.146 4.54A.5.5 0 0 0 0 4.893v6.214a.5.5 0 0 0 .146.353l4.394 4.394a.5.5 0 0 0 .353.146h6.214a.5.5 0 0 0 .353-.146l4.394-4.394a.5.5 0 0 0 .146-.353V4.893a.5.5 0 0 0-.146-.353zm-6.106 4.5L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 1 1 .708-.708"/>
							</svg>
						</span>
						<span className="mx-auto text-lg md:text-xl text-shadow-lg/90 text-shadow-gray-400">No se encontró el artículo. Inténtelo mas tarde.</span>
						<button type="button" onClick={() => navigate('/')} className="px-2.5 py-2 rounded-md hover:scale-110 transition delay-100 duration-200 ease-in-out hover:bg-conic/decreasing from-violet-700 via-lime-300 to-violet-700 mx-auto bg-green-500 hover:text-white" >Regresar al Inicio</button>
					</div>
				)
